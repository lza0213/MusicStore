...
namespace MVCMusicStore2019
{
    ...
    public class IdentityConfig
    {
        ...
        {
            //CreatePerOwinContext用于创建AppUserManager的新实例，AppIdentityDbContext类用于每一个请求。
            app.CreatePerOwinContext(Infrastructure.MusicDbContext.Create);
            app.CreatePerOwinContext<ApplicationUserManager>(ApplicationUserManager.Create);
            app.CreatePerOwinContext<ApplicationRoleManager>(ApplicationRoleManager.Create);

            //告诉ASP.NET Identity如何用Cookie去标识已认证的用户，以及通过CookieAuthenticationOptions类指定的选项在哪儿。
            app.UseCookieAuthentication(new CookieAuthenticationOptions
            {
                AuthenticationType = DefaultAuthenticationTypes.ApplicationCookie,
                //指定了一个URL，这是未认证客户端请求内容时要重定向的地址。
